...
def bubble_sort(arr: List[T], asc: bool = True) -> None:
    """冒泡排序
    ...
    Args:
        arr (List[T]): 待排序数组
        asc (bool, optional): 是否升序排序. Defaults to True.
    """
    n: int = len(arr)
    if asc: # 升序排序
        for i in range(n - 1, 0, -1): # 外循环：未排序区间为 [0, i]
            for j in range(i): # 内循环：将未排序区间 [0, i] 中的最大元素交换至该区间的最右端
                if arr[j] > arr[j + 1]:
                    arr[j], arr[j + 1] = arr[j + 1], arr[j]
    else: # 降序排序
        for i in range(n - 1, 0, -1): # 外循环：未排序区间为 [0, i]
            for j in range(i): # 内循环：将未排序区间 [0, i] 中的最小元素交换至该区间的最右端
                if arr[j] < arr[j + 1]:
                    arr[j], arr[j + 1] = arr[j + 1], arr[j]

def short_bubble_sort(arr: List[T], asc: bool = True) -> None:
    """短冒泡排序
...
def selection_sort(arr: List[T], asc: bool = True) -> None:
    """选择排序
...
def insertion_sort(arr: List[T], asc: bool = True) -> None:
    """插入排序
...
def shell_sort(arr: List[T], asc: bool = True) -> None:
    """希尔排序
...
def _merge(arr: List[T], left: int, mid: int, right: int, asc: bool) -> None:
    ...
    Args:
        ...
        asc (bool): 是否升序排序
    """
    tmp: List[Optional[T]] = [None] * (right - left + 1) # 创建一个临时数组 tmp ，用于存放合并后的结果
    i: int = left; j: int = mid + 1; k: int = 0 # 初始化左子数组、右子数组和临时数组的起始索引
    if asc: # 升序排序
    ...
    for k in range(len(tmp)):
        arr[left + k] = tmp[k] # type: ignore

def merge_sort(arr: List[T], left: int, right: int, asc: bool = True) -> None:
    """归并排序
    ...
    # 合并阶段
    _merge(arr=arr, left=left, mid=mid, right=right, asc=asc)



def _partition(arr: List[T], left: int, right: int, asc: bool) -> int:
    """快速排序的辅助函数（哨兵划分），用于确定基准数应在有序数组中的位置

    Args:
        arr (List[T]): 外部的待排序数组
        left (int): 基准数当前在数组中的索引（子数组的索引起点）
        right (int): 子数组的索引终点
        asc (bool): 是否升序排序
    ...
    Returns:
        int: 基准数应在有序数组中的索引
    """
    pivot: T = arr[left] # 基准数，此时将索引 left 视为一个空的坑
    if asc: # 升序排序
        while left < right:
...
def quick_sort(arr :List[T], left: int, right: int, asc: bool = True) -> None:
    """快速排序
...
def bucket_sort(arr: List[int], k: int = 10, asc: bool = True) -> List[int]:
    ...
    Args:
        arr (List[int]): 待排序数组
        k (int, optional): 桶的个数. Defaults to 10.
        asc (bool, optional): 是否升序排序. Defaults to True.
    ...
    for i in range(1, len(arr)):
        ...
        if arr[i] < minimum:
            minimum = arr[i]
    buckets: List[List[int]] = [[] for _ in range(k)] # 无法事先预知每个桶中会存放多少元素，需使用动态数组（自定义的 DynamicArray 类没有实现 sort 方法，用列表来代替）
    for num in arr:
    ...
    result: List[Optional[int]] = [None] * len(arr)
    idx: int = 0
    if asc: # 升序
...
def radix_sort(arr: List[int], d: int = 10, asc: bool = True) -> List[int]:
    ...
    Args:
        arr (List[int]): 待排序数组
        d (int, optional): 元素使用的进制. Defaults to 10.
        asc (bool, optional): 是否升序排序. Defaults to True.
